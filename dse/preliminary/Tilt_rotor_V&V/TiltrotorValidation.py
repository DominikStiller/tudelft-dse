...
def RotorSizingValidation():
    ...
    ReferenceRadius, ReferenceWeight, ReferencePower, ReferenceNrotor, ReferenceNblade, ReferenceTipSpeed = ([],)*6
    i=0
    for elem in datas:
    ...
    for i in range(3):
        ...
        R, T, hp,P, m = RadiusMassElementMomentum(M=ReferenceWeight[i], N_rotors=ReferenceNrotor[i], N_blades=ReferenceNblade[i],
                                  coaxial=False, V_tip=ReferenceTipSpeed[i] * const['soundSpeed'], print_results=False)
...
def DragValidation():
    print("The drag estimations stem from Torenbeek's book: Synthesis of Subsonic Airplane Design.\n As such, the methods are deemed valid")
...
def PowerSizingValidation():
    ...
    #Thrust Power:
    P_TO = aircraftParameters['totalRotors']*takeOffThrust*np.sqrt(takeOffThrust/(2*rho*A))
    ...
    if TO_battery*EnergyDensity >= E_cruise+2*E_TO:
        Cruise_battery = max((E_cruise-(TO_battery*EnergyDensity - E_TO))/EnergyDensity, P_cruise/PowerDensity)

    RefBattery = (TO_battery+Cruise_battery)*1.3/0.95

    calcBatteryMass, panelMass, powerSurplus = size_power_subsystem(rotorRadius, takeOffThrust, cruiseThrust, cruiseTime, takeOffTime, surfaceArea, plot=False, Print=False)
    print(f"Difference between calculated and required battery mass: {100*(RefBattery-calcBatteryMass)/RefBattery}%")
