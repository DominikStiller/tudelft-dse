...
filenameArray = ['Flying_Wing_Data_AoA=9_3.csv', 'Bi_Wing_1_a=14.00_v=111.00ms.csv', 'Bi_Wing_2_a=14.00_v=111.00ms.csv', 'S1223.csv']
n = 10000
...
def GetWingbox(Indx): # 0 and 1 for airfoil wingbox, 2 for square wingbox
    ...
    if Indx == 2:
        ...
        WingboxArray[1] = np.array([0.07, 0.12486, 0.04953, -0.01123])  # [-] As percentage of unit chord
    ...
    for i in range(len(WingboxArray[0]) - 1):
        b_lst.append(((WingboxArray[0, i] - WingboxArray[0, i + 1]) ** 2 + (WingboxArray[1, i] - WingboxArray[1, i + 1]) ** 2) ** 0.5)
    b_lst.append(((WingboxArray[0, 0] - WingboxArray[0, -1]) ** 2 + (WingboxArray[1,0] - WingboxArray[1, -1]) ** 2) ** 0.5)
...
def NeutralAxisUnit(WingboxArray):
    xbar = sum(WingboxArray[3,i] * WingboxArray[2, i] * WingboxArray[4,i] for i in range(len(WingboxArray[3]))) / sum(
        WingboxArray[3, i] * WingboxArray[2,i] for i in range(len(WingboxArray[3])))
    ybar = sum(WingboxArray[3,i] * WingboxArray[2, i] * WingboxArray[5,i] for i in range(len(WingboxArray[3]))) / sum(
        WingboxArray[3, i] * WingboxArray[2,i] for i in range(len(WingboxArray[3])))
...
def IxxCalcUnit(WingboxArray, NA):  # Multiply by c^3 to gain the moment of inertia at said location
    Ixx12 = (WingboxArray[2,0] * WingboxArray[3,0] ** 3 * ((WingboxArray[1,1] - WingboxArray[1,0]) / WingboxArray[3,0]) ** 2) / 12
    Ixx23 = (WingboxArray[2,1] * WingboxArray[3,1] ** 3) / 12
    Ixx34 = (WingboxArray[2,2] * WingboxArray[3,2] ** 3 * ((WingboxArray[1,3] - WingboxArray[1,2]) / WingboxArray[3,2]) ** 2) / 12
    Ixx41 = (WingboxArray[2,3] * WingboxArray[3,3] ** 3) / 12
    IxxPart = [Ixx12, Ixx23, Ixx34, Ixx41]
    Ixx = sum(IxxPart[i] + WingboxArray[2,i] * WingboxArray[3,i] * (WingboxArray[5,i] - NA[1]) ** 2 for i in range(len(IxxPart)))
    return Ixx


def IyyCalcUnit(WingboxArray, NA):  # Multiply by c^4 to gain the moment of inertia at said location
    Iyy12 = (WingboxArray[2,0] * WingboxArray[3,0] ** 3 * (
            (WingboxArray[0,1] - WingboxArray[0,0]) / WingboxArray[3,0]) ** 2) / 12
    Iyy23 = 0
    Iyy34 = (WingboxArray[2,2] * WingboxArray[3,2] ** 3 * (
            (WingboxArray[0,3] - WingboxArray[0,2]) / WingboxArray[3,2]) ** 2) / 12
    Iyy41 = 0
    IyyPart = [Iyy12, Iyy23, Iyy34, Iyy41]
    Iyy = sum(IyyPart[i] + WingboxArray[2,i] * WingboxArray[3,i] * (WingboxArray[4,i] - NA[0]) ** 2 for i in range(len(IyyPart)))
    return Iyy


def IxyCalcUnit(WingboxArray, NA):  # Multiply by c^4 to gain the moment of inertia at said location
    Ixy12 = (WingboxArray[2,0] * WingboxArray[3,0] ** 3 * (
                ((WingboxArray[1,1] - WingboxArray[1,0]) * (WingboxArray[0,1] - WingboxArray[0,0])) / (WingboxArray[3,0] ** 2))) / 12
    Ixy23 = 0
    Ixy34 = (WingboxArray[2,2] * WingboxArray[3,2] ** 3 * (
                ((WingboxArray[1,3] - WingboxArray[1,2]) * (WingboxArray[0,3] - WingboxArray[0,2])) / (WingboxArray[3,2] ** 2))) / 12
    Ixy41 = 0
    IxyPart = [Ixy12, Ixy23, Ixy34, Ixy41]
    Ixy = sum(IxyPart[i] + WingboxArray[2,i] * WingboxArray[3,i] * (WingboxArray[5,i] - NA[1]) * (WingboxArray[4,i] - NA[0]) for i in
              range(len(IxyPart)))
    return Ixy
...
def InternalStress(Loads, WingboxArray, Data, NA):
    ...
    sigmay_1_lst = [10 ** -6 * ((Loads[0,i] * Iyy - Loads[1,i] * Ixy) * (-WingboxArray[1,0] + NA[1]) + (
                Loads[1,i] * Ixx - Loads[0,i] * Ixy) * -(-WingboxArray[0,0] + NA[0])) / (
                                (Ixx * Iyy - Ixy ** 2) * Data[1,i] ** 2) for i in range(len(Data[0]))]
    sigmay_2_lst = [10 ** -6 * ((Loads[0,i] * Iyy - Loads[1,i] * Ixy) * (-WingboxArray[1,1] + NA[1]) + (
...
def MakePlots0(indx, WingboxArray, Data, Loads, NA):
    ...
    ax.plot([WingboxArray[0,0], WingboxArray[0,-1]], [WingboxArray[1,0], WingboxArray[1,-1]], color='black', zorder=4)
    n = [1, 2, 3, 4]
...
def MakePlots2(indx, WingboxArray, Data, Loads, NA):
    ...
    plt.xlabel('Span [m]')
    plt.ylabel('Mx [Nm]')
    plt.plot((Data[0])*-1, Loads[0], color=color)
    plt.show()
