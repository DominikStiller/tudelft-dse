...
def size_power_subsystem(
    ...
    try:
        percentRechargeable = np.count_nonzero(~np.isnan(rechargeCapability)) / np.size(
            rechargeCapability
        )
    except:
        ...
        if Print:
            print(
                f"We cannot recharge in flight - average recharge time landed is {np.mean(rechargeTime)}"
            )
    ...
    if Print:
        ...
        print(
            f"Recharge is possible in {100 * percentRechargeable}% of the planet while in cruise\n"
        )

    # Size the batteries and the arrays
    energyConsumption = 2 * takeOffEnergy + cruiseEnergy
    batteryMass = max(
        2 * takeOffEnergy / const["takeoffBatteryEnergyDensity"],
        takeOffPower / const["takeoffBatteryPowerDensity"],
    )
...
if __name__ == "__main__":
    ...
    size_power_subsystem(
        rotorRadius, takeOffThrust, cruiseThrust, cruiseTime, takeOffTime, collectingArea, plot=True
    )
