...
def max_rotor_loads(airfoil_shape):
    ...
    fill_factor = 0.1
    area, length = airfoilArea()
    while np.any(np.abs(max_stress)) > const['allowedStress']:
        aircraftParameters['rotorMass'] *= (fill_factor + 0.01)/fill_factor
        length *= (fill_factor + 0.01)/fill_factor
        fill_factor += 0.01
    ...
    print(f'Fill factor: {fill_factor-0.01}')
    print(f'Thickness: {area/length}')
