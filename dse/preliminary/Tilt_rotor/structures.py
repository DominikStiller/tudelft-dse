...
def airfoilArea():
    ...
    length = np.sum(
        np.sqrt((x_cord_top[1:] - x_cord_top[:-1]) ** 2 + (y_cord_top[1:] - y_cord_top[:-1]) ** 2)
    ) + np.sum(
        np.sqrt(
            (x_chord_bottom[1:] - x_chord_bottom[:-1]) ** 2
            + (y_chord_bottom[1:] - y_chord_bottom[:-1]) ** 2
        )
    )
...
def calculate_cg():
    ...
    locations = np.array(
        [
            ...
            fuselageLength / 2,
            cockpitLength
            + aircraftParameters["chord"] / 2
            + 1.25 * aircraftParameters["rotorRadius"],
            fuselageLength / 2,
            cockpitLength + aircraftParameters["chord"] / 2,
            cockpitLength / 2,
        ]
...
def max_wing_loads():
    F_ay = const["gravityMars"] * (
        aircraftParameters["rotorMass"]
        + aircraftParameters["wingMass"]
        - const["takeOffLoad"] * aircraftParameters["totalMass"] / 2
    )
    F_az = (
        -const["gravityMars"]
        ...
        * (
            ...
            * (
                aircraftParameters["wingMass"]
                - const["takeOffLoad"] * aircraftParameters["totalMass"]
            )
            + aircraftParameters["rotorMass"]
        )
    )
    ...
    M_az = (
        ...
        * aircraftParameters["cd"]
        / aircraftParameters["cl"]
        * aircraftParameters["wingspan"]
        / 4
    )
...
def max_rotor_loads(airfoil_shape):
    ...
    F_z = F_z_init = (
        initial_mass * const["gravityMars"]
        - (const["takeOffLoad"] * const["gravityMars"] * const["maxMass"]) / 24
    )
    ...
    while np.max(np.abs(max_stress)) > const["allowedStress"]:
        fill_factor += 0.0001
        guess_Rotor_mass = (
            const["bladeDensity"] * aircraftParameters["rotorRadius"] * initial_area * fill_factor
        )
        ...
        F_z = (
            guess_Rotor_mass * const["gravityMars"]
            - (const["takeOffLoad"] * const["gravityMars"] * const["maxMass"] / 0.88) / 24
        )
        ...
        Ixz = (area / np.size(airfoil_shape[0])) * np.sum(
            (airfoil_shape[0] - x_av) * (airfoil_shape[1] - z_av)
        )

        max_stress = (
            (Mx * Izz - Mz * Ixz) * (airfoil_shape[1] - z_av)
            + (Mz * Ixx - Mx * Ixz) * (airfoil_shape[0])
            - x_av
...
def assembly_volume():
    ...
    fuselageVolume = (
        1 / 3 * cockpitLength * np.pi * (fuselageWidth / 2) ** 2
        + 2 * np.pi * fuselageWidth / 2 * aircraftParameters["chord"]
    )
    ...
    rotorVolume = (
        ...
        * (Area_top - Area_bot)
        * (aircraftParameters["rotorRadius"]) ** 2
        / 20
    )
