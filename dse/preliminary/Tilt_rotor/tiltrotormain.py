...
from dse.preliminary.Tilt_rotor.PerformanceAnalysis import (
    PayloadRange,
    AircraftClimbPerf,
    RotorClimbPerf,
)
...
def design(
    iterate=False, Print=False, inputSensitivity=(None, None), functionSensitivity=(None, None)
):
    ...
    while diff > 0.005 and Range >= 1000:
        ...
        (
            ...
            aircraftParameters["horsepowerPerEngine"],
            aircraftParameters["totalPower"],
            aircraftParameters["rotorMass"],
        ) = RadiusMassElementMomentum(
            M=Mass_thrust,
            N_rotors=aircraftParameters["totalRotors"],
        ...
        if functionSensitivity[0] == 0:
            aircraftParameters["totalPower"] *= functionSensitivity[1]
            aircraftParameters["rotorMass"] *= functionSensitivity[1]
            aircraftParameters["rotorRadius"] *= functionSensitivity[1]
            takeOffThrustPerEngine *= functionSensitivity[1]
            aircraftParameters["horsepowerPerEngine"] *= functionSensitivity[1]

        # Calculate wing area
        area(const["cl"], Mass_design, 0.5 * const["airDensity"] * const["cruiseSpeed"] ** 2)
        if functionSensitivity[0] == 1:
            aircraftParameters["wingArea"] *= functionSensitivity[1]

        cd0 = DragEstimation(aircraftParameters["wingArea"], const["cruiseSpeed"], const["visc_cr"])
        if functionSensitivity[0] == 2:
            cd0 *= functionSensitivity[1]

        aircraftParameters["cruiseThrust"] = (
            (cd0 + const["cl"] ** 2 / (np.pi * aircraftParameters["AR"] * const["oswald"]))
            * 0.5
            * const["airDensity"]
            * const["cruiseSpeed"] ** 2
        ...
        if functionSensitivity[0] == 3:
            ...
            aircraftParameters["panelMass"] *= functionSensitivity[1]
        ...
        if functionSensitivity[0] == 4:
            ...
            aircraftParameters["wingMass"] *= functionSensitivity[1]
            aircraftParameters["tailMass"] *= functionSensitivity[1]
            aircraftParameters["bodyMass"] *= functionSensitivity[1]

        # print(f'Maximum Range available: {Range}[km]')

        aircraftParameters["totalMass"] = (
            aircraftParameters["rotorMass"]
            + aircraftParameters["panelMass"]
            + aircraftParameters["batteryMass"]
            + aircraftParameters["wingMass"]
        ...
        if (
            aircraftParameters["totalMass"] > Mass_design
            and functionSensitivity == (None, None)
            and inputSensitivity == (None, None)
        ):
        ...
        ROC_rotor = RotorClimbPerf(
            Mass_design, aircraftParameters["rotorRadius"], aircraftParameters["totalRotors"]
        )
        ...
        if Print:
            ...
            print(
                f'Cg location is {1.78 + aircraftParameters["chord"] / 2 - calculate_cg()} in front of the rotors thrust'
            )
