...
class System:
    def __init__(self):
        ...
        self.velocity_linear_prev = np.array([0, 0, 0.0])
...
dt = 0.01
system = System()
controller_thrust = Controller(dt=dt, Kp=6000.0, Ki=1600.0, Kd=1000.0)
controller_pitch = Controller2(dt=dt, Kp=-25000.0, Ki=-5000.0, Kd=-25000.0)

euler_ref = np.array([0, 0, 0.0])
velocity_linear_ref = np.array([0, 0, -2.0])
...
plt.plot(
    np.array(np.arange(0, 1e4, 1)) * dt,
    thrust_in_time[:, 2],
    label="Thrust in z-direction (downward positive)",
)
