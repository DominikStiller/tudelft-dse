...
class Coefficients:
    ...
    def loading_diagram(self):
        ...
        assert min(cgs) >= min(
            self.location
        ), f"The forwards cg is in front of the foremost component"
        return cgs

    def tail_area(self, cgs):
        cgrange = np.arange(0, 1, 0.01)
        sh_s_stability = (cgrange - (self.X_ac - self.SM)) / (
            (self.CL_alpha_h / self.CL_alpha_A)
            * (1 - self.downwash_angle)
            * (self.length_h / self.main_wing_chord)
            * self.Vh_V**2
        )
        sh_s_control = (cgrange + self.Cmac / self.CL_A_h - self.X_ac) / (
            (self.CL_h / self.CL_A_h) * (self.length_h / self.main_wing_chord) * self.Vh_V**2
        )
        ...
        sh_s_control_value = (min(cgs) + self.Cmac / self.CL_A_h - self.X_ac) / (
            (self.CL_h / self.CL_A_h) * (self.length_h / self.main_wing_chord) * self.Vh_V**2
        )
        ...
        assert (
            sh_s_value > 0
        ), f"The tail can not have negative volume, check the signs of the coefficients"
        assert (
            sh_s_value < 1
        ), f"The tail should be smaller than the main wing, check the given coefficients"
    ...
    def neutral_stick_fixed(self, areat):
        ...
        x_w = self.X_ac
        x_n_fix = (C_N_h_alpha / C_N_alpha) * (1 - depsilon_dalpha) * V_h_V_ratio**2 * (
            Sh_S_ratio * l_h / c
        ) + x_w
    ...
    def neutral_stick_free(self, areat):
        ...
        x_n_free = (C_N_h_alpha_free / C_N_alpha) * (1 - depsilon_dalpha) * (V_h_V_ratio**2) * (
            Sh_S_ratio * l_h / c
        ) + x_w
    ...
    def elevator_force(self, cg_chord, xn_free_chord, xn_fixed_chord, area_tail):
        C_m_delta = (
            -self.C_N_h_delta * self.Vh_V**2 * area_tail * self.length_h / self.main_wing_chord
        )
        ...
        Fe = (
            ...
            * self.Vh_V**2
            * (
                self.W0 * self.C_h_delta * (cg_chord - xn_free_chord) / (self.S * C_m_delta)
                - 0.5 * self.rho * V**2 * self.C_h_delta_t * single_deflection
            )
        )
        ...
        control_force = (
            ...
            * (
                self.W0 * self.C_h_delta * cg_chord * xn_free_chord / (self.S * C_m_delta)
                - 0.5 * self.rho * self.vel**2 * self.C_h_delta_t * single_deflection
            )
        )
        elevator_deflection = (-1 / C_m_delta) * (
            self.C_m_0 + self.W0 * (cg_chord - xn_fixed_chord) / (0.5 * self.rho * V**2 * self.S)
        )
        cruise_elevator_deflection = (-1 / C_m_delta) * (
            self.C_m_0
            + self.W0 * (cg_chord - xn_fixed_chord) / (0.5 * self.rho * self.vel**2 * self.S)
        )
        print(
            f"At the velocity of {self.vel}, the elevator trim: {np.degrees(cruise_elevator_deflection)}"
        )
...
class Equilibrium:
    def __init__(self):
        ...
        self.X_W = (
            max(cg) - coefficients.X_ac
        ) * coefficients.main_wing_chord  # X location of main wing ac
    ...
    def sum_in_x(self):
        tx = (
            self.C_T_b * (self.Sb / self.S) + self.C_T_h * self.Sh_S * self.Vh_V**2 + self.C_T_w
    ...
    def sum_in_z(self):
        tz = (self.C_N_w + self.C_N_h * self.Sh_S * self.Vh_V**2) * (
            0.5 * self.rho * self.S * self.vel**2
        ) - self.W0 * np.cos(self.pitch)
        print(f"tz is {tz}")
        return tz
    ...
    def sum_moments(self, Tx, Tz):
        ...
        tail = (self.C_M_ac_h * self.Chord_h - self.C_T_h * z_h) * (
        ...
        momenttail = (self.C_M_ac_h * self.Chord_h - self.C_T_h * z_h + self.C_N_h * x_h) * (
            (self.Vh_V**2) * self.Sh_S / self.Chord_w
        )
    ...
    def rudder_sizing(self, x_rudder):
        ...
        cl_due_to_sideslip = (
            side_slip_angle * self.cl_alpha_rud
        )  # or just get the exact number at the sideslip angle
        ...
        d_fuselage = (
            0.5 * self.rho * s_fuselage_side * drag_coefficient_fuselage * self.v_cross_wind**2
        )
