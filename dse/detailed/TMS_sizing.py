...
Arad_out = [
    fbo * Abat_outside + fbi * Abat_inside + fw * Aw,
    fbo * Abat_outside + fbi * Abat_inside + fw * Aw,
]
Aconv = [
    fbi * Abat_inside + fbo * Abat_outside + fbaf * Abat_front_aft + fw * Aw,
    0,
]  # fbi * Abat_inside + fbo * Abat_outside + fbaf * Abat_front_aft + fw * Aw]
...
# Electronics
Q_in_int_electronics = (
    25 + 25
)  # [W] # 25 -> flight computer + 25 assumed for the rest of electrical
...
Tac_h = calc_eqm_temperature(
    Q_in_solar=Q_in_solar_h,
...
Q_out_rad_h = calc_heat_out_radiation(
    epsilon_ac=epsilon_ac, Tac=Tac_h[-1].real, Tamb=Tamb[0], Arad=Arad_out[0]
)
...
Tac_c = calc_eqm_temperature(
    Q_in_solar=Q_in_solar_c,
...
Q_out_rad_c = calc_heat_out_radiation(
    epsilon_ac=epsilon_ac, Tac=Tac_h[-1].real, Tamb=Tamb[1], Arad=Arad_out[1]
)
...
Aradiator = (
    2 * (A_radial + A_circ) * 2
)  # The last *2 accounts for both sides of a cell wall cooling
...
Q_out_conv_h = calc_heat_out_convection(h=h[0], Tac=Tac_h[-1].real, Tamb=Tamb[0], Aconv=Aconv[0])
Q_out_rad_h = calc_heat_out_radiation(
    epsilon_ac=epsilon_ac, Tac=Tac_h[-1].real, Tamb=Tamb[0], Arad=Arad_out
)
...
Q_out_conv_c = calc_heat_out_convection(h=h[1], Tac=Tac_h[-1].real, Tamb=Tamb[1], Aconv=Aconv[1])
Q_out_rad_c = calc_heat_out_radiation(
    epsilon_ac=epsilon_ac, Tac=Tac_h[-1].real, Tamb=Tamb[1], Arad=Arad_out
)
...
pipe_volume = (
    1.5 * span * 2 * 2 * pipe_crosssectional_area
)  # 1.5 safety factor, 2 * 2 because 2 times half a wing for full wing and 2 times to go to engine and come back to fuselage
