...
def solve(fn, **kwargs):
    ...
    default_args = {
        k for k, v in signature.parameters.items() if v.default is not inspect.Parameter.empty
    }
    arg_to_solve = (
        signature.parameters.keys() - kwargs.keys() - default_args
    )  # Find arguments that have been passed
...
class Tank:
    ...
    def _calculate_properties_after_setting_subclass(self):
        ...
        self.SF_LBB = self.material_Kic / (
            self.content_pressure * self.diameter / 2 / self.t_cyl / 2 * np.sqrt(np.pi * self.t_cyl)
        )
...
class GasTank(Tank):
    def __init__(self, *, gas_M, **kwargs):
        ...
        self.content_rho = solve(
            idealgas, p=self.content_pressure, T=self.content_temperature, R=self.gas_R
        )
        self.volume = self.content_mass / self.content_rho
        self.length = calc_length_tank(V=self.volume, d=self.diameter)

        self.t_cyl = solve(
            hoop_stress_cyl, sigma_h=self.material_sigma_y, p=self.content_pressure, d=self.diameter
        )
        self.t_sph = solve(
            hoop_stress_sph, sigma_h=self.material_sigma_y, p=self.content_pressure, d=self.diameter
        )

        self.empty_mass = calc_thin_cyl_mass(
            l=self.length,
            d=self.diameter,
            t1=self.t_cyl,
            t2=self.t_sph,
...
class LifeSupportSystem:
    def __init__(
        ...
        self.required_rate_air_LSS = (
            self.Astronauts.required_rate_air + self.air_leakage_rate
        ) * self.num_astronauts
        ...
        self.total_mass = (
            self.required_mass_air
            + self.required_mass_water
            + self.O2Tank.empty_mass * self.num_tanks_air
            + self.H2OTank.empty_mass * self.num_tanks_water
        )
        self.empty_mass = (
            self.O2Tank.empty_mass * self.num_tanks_air
            + self.H2OTank.empty_mass * self.num_tanks_water
