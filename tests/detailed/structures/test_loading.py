...
class TestBeam(TestCase):
    def test_add_loading_point_load(self):
        ...
        point_load = Force(magnitude=np.array([[0, 0, F]]).T,
                           point_of_application=np.array([[0, loc, 0]]).T)
    ...
    def test_add_loading_distributed_load(self):
        ...
        if np.all(wing.f_loading[y_index_1:] == np.reshape(np.sum(F, 1), (3, 1))) and \
                np.all(wing.f_loading[y_index_2:y_index_1] == np.reshape(np.sum(F[:, 1:], 1), (3, 1))) and \
                np.all(wing.f_loading[y_index_3:y_index_2] == np.reshape(F[:, -1], (3, 1))) and\
                np.all(wing.f_loading[:y_index_3] == 0):
            if np.abs(wing.m_loading[-1][0]) - 2.6 < 0.01 and \
                np.abs(wing.m_loading[y_index_1][0]) - 2.3 < 0.01 and \
                np.abs(wing.m_loading[y_index_2][0]) - 0.5 < 0.01 and \
                np.all(wing.m_loading[:y_index_3] == 0):
                assert True
            else:
            ...
            assert np.all(wing.f_loading[y_index_1:] == np.reshape(np.sum(F, 1), (3, 1))), 'Forces at root are off'
            assert np.all(wing.f_loading[y_index_2:y_index_1] == np.reshape(np.sum(F[:, 1:], 1), (3, 1))), 'Forces at middle are off'
            assert np.all(wing.f_loading[y_index_3:y_index_2] == np.reshape(F[:, -1], (3, 1))), 'Forces at semi-middle are off'
