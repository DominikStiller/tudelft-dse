...
class TestBeam(TestCase):
    ...
    def test_add_loading_point_load(self):
        ...
        point_load = Force(
            magnitude=np.array([[0, 0, F]]).T,
            point_of_application=np.array([[x / 2, loc, z / 2]]).T,
        )
    ...
    def test_add_loading_distributed_load(self):
        ...
        # Check moments
        assert_allclose(
            wing.m_loading[-1][0], -2.6, rtol=0.05, err_msg="Moment at the tip is wrong"
        )
        assert_allclose(
            wing.m_loading[y_index_1][0], -2.3, rtol=0.05, err_msg="Moment at segment 1 is wrong"
        )
        assert_allclose(
            wing.m_loading[y_index_2][0], -0.5, rtol=0.05, err_msg="Moment at segment 2 is wrong"
        )
        assert_allclose(
            wing.m_loading[y_index_3 - 1][0],
            0,
            rtol=0.05,
            err_msg="Moment at the third point is wrong",
        )

        # Check forces
        self.assertTrue(np.all(wing.f_loading[y_index_1:] == np.reshape(np.sum(F, 1), (3, 1))))
        self.assertTrue(
            np.all(wing.f_loading[y_index_2:y_index_1] == np.reshape(np.sum(F[:, 1:], 1), (3, 1)))
        )
        self.assertTrue(np.all(wing.f_loading[y_index_3:y_index_2] == np.reshape(F[:, -1], (3, 1))))

    def test_InternalLoads_Rhombus(self):
        ## Hand Calculations
        NAx_hand = 0.0  # Neutral axis as obtained by hand calculations
        NAz_hand = 0.0  # Neutral axis as obtained by hand calculations
        Ixx_hand = 0.0375  # [m4]
        Izz_hand = 0.0375  # [m4]
        Ixz_hand = 0.000  # [m4]
        sigma_hand = (
        ...
        Applied_Load = Force(
            ...
            point_of_application=np.array([[0], [-b], [0]]),
        )
        rombus.add_loading(Applied_Load)
        # rombus.plot_internal_loading()

        x_booms_nr, z_booms_nr = np.split(
            np.reshape(rombus.section, (np.size(rombus.y), 2 * np.shape(rombus.x)[0])), 2, 1
        )
        ...
        NAx, NAz = rombus.NeutralAxis(
            boomArea_nr=np.ones((8, np.size(l))) * 0.0125,
            x_booms_nr=x_booms_nr,
            z_booms_nr=z_booms_nr,
        )
        ...
        assert_allclose(
            sigma_nr[:, -1], sigma_hand.T[0], rtol=1e-3, err_msg="The stresses are wrong"
        )
    ...
    def test_InternalLoads_Parallelogram(self):
        ...
        Ixz_hand = 0.0375  # [m4]
        sigma_hand = (
            np.array([[-2.4, 0.133333, 2.6666, 2.5333, 2.4, -0.13333, -2.666, -2.53333]]).T * 1e6
        )
        b = 10
        ...
        x_booms_nr, z_booms_nr = np.split(
            np.reshape(
                parallelogram.section, (np.size(parallelogram.y), 2 * np.shape(parallelogram.x)[0])
            ),
            2,
            1,
        )
        ...
        NAx, NAz = parallelogram.NeutralAxis(
            boomArea_nr=np.ones((8, np.size(l))) * 0.0125,
            x_booms_nr=x_booms_nr,
            z_booms_nr=z_booms_nr,
        )
        Ixx, Izz, Ixz = parallelogram.MoI(
            boomArea_nr=np.ones((8, np.size(l))) * 0.0125,
            x_booms_nr=x_booms_nr,
            z_booms_nr=z_booms_nr,
        )
    ...
    def test_ShearLoads_Wingbox(self):
        ## Hand Calculations
        q_total_hand = (
            np.array([[12.72, -5.32, -34.18, -37.79, -34.18, -5.32, 12.72, 17.05]]).T
            * 1e3
            * np.ones(100)
        )
        b = 10
        x = np.array([[0.6, 0.36, 0.12, 0, 0, 0.12, 0.36, 0.6, 0.6]])
        z = np.array([[0.03, 0.1, 0.1, 0.05, -0.05, -0.1, -0.1, -0.03, 0.03]])
        l = np.linspace(-b, 0, 100)
        boomArea_nr = np.array(
            [
                [
                    0.0002,
                    0.00025,
        ...
        Applied_Load = Force(
            ...
            point_of_application=np.array([[0.12], [-b], [0]]),
        )
        wingbox.add_loading(Applied_Load)

        x_booms_nr, z_booms_nr = np.split(
            np.reshape(wingbox.section, (np.size(wingbox.y), 2 * np.shape(wingbox.x)[0])), 2, 1
        )
        if np.all(x_booms_nr[:, 0] == x_booms_nr[:, -1]):
            x_booms_nr = x_booms_nr[:, :-1]
            z_booms_nr = x_booms_nr[:, :-1]
    ...
    def test_stress_calculations(self):
        ...
        dummy_force_1 = Force(
            magnitude=np.array([[0], [1e3], [0]]), point_of_application=np.array([[0], [-5], [0]])
        )
        rombus.add_loading(dummy_force_1)

        stress = rombus.StressCalculations(boomArea_nr=A)

        self.assertTrue(
            np.all(stress == stress[0]),
            msg="The stress on the booms due to a point load along the length "
            "does not result in a constant stress",
        )
        rombus.unload()

        dummy_force_2 = Force(
            magnitude=np.array([[0], [0], [1e3]]), point_of_application=np.array([[0], [-5], [0]])
        )
    ...
    def test_internal_stress(self):
        ...
        self.assertTrue(
            np.all(rombus.sigma <= 250e6 / 1.5), msg="Maximum stress is higher than allowed"
        )
    ...
    def test_masses_constant_section(self):
        ...
        assert_allclose(
            np.sum(masses), A * y * materials["Al/Si"].rho, rtol=1e-4, err_msg="Total mass is wrong"
        )
    ...
    def test_design_joint(self):
        ...
        P_mag = 1e5
        P = Force(
            magnitude=np.array([[0, 0, P_mag]]).T,
            point_of_application=np.array([[x0 / 2], [-y], [z0 / 2]]),
        )

        squareBeam.add_loading(P)
        indx = (np.abs(squareBeam.y + y / 2)).argmin()
        F = np.abs(squareBeam.f_loading[indx][1]) + np.abs(squareBeam.m_loading[indx][0]) * np.max(
            np.abs(squareBeam.z[:, indx] - squareBeam.fix[1, indx])
        )
    ...
    def test_buckling(self):
        ...
        s_analytic = (
            ...
            * np.pi**2
            * materials["Al/Si"].E
            / (12 * (1 - materials["Al/Si"].poisson ** 2))
            * (0.005 / 0.1) ** 2
        )
...
class ValidateBeam(TestCase):
    def test_internal_load(self):
        ...
        positions0 = np.array([[-0.6], [-0.3], [-0.15]])

        positions1 = np.array([[-0.15, -0.3, -0.6], [-0.15, -0.3, -0.6], [-0.15, -0.3, -0.6]])

        Ra0 = np.array([[3.33], [6.67], [8.33]])

        Ra1 = np.array([[9.17], [18.33], [27.5]])
        ...
        for i in range(3):
            ...
            loading0 = Force(
                ...
                point_of_application=np.array([[0.025], [positions0[i][0]], [0.025]]),
            )
            reaction = Force(
            ...
            assert_allclose(
                beam.f_loading[-1][-1], Rb0[i], rtol=0.01, err_msg="Reaction force does not match"
            )
            ...
            loading1 = Force(
                ...
                point_of_application=np.vstack(
                    (0.025 * np.ones(3), positions1[i], 0.025 * np.ones(3))
                ),
            )
            reaction = Force(
