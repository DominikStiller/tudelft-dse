...
class TestFuselageLogic(unittest.TestCase):
    ...
    def test_layer3_assign_properties(self):
        ...
        assert_equal_message_firsttest_2 = (
            "The fuselage's total length is not being calculated properly"
        )
        assert_equal_message_firsttest_3 = (
            "The fuselage's total surface is not being calculated properly"
        )
    ...
    def test_layer2_check_methods(self):
        ...
        self.assertEqual(
            round(fus.calculate_cd(), 5),
            0.02941,
            "The method calculate_cd is failing to calculate correctly.",
        )
    ...
    def test_layer4_check_drag_calculation(self):
        ...
        self.assertEqual(
            round(drag, 2),
            46.88,
            "The final drag force calculation does not return the correct value",
        )
    ...
    def test_static_methods(self):
        ...
        self.assertEqual(
            Fuselage.area_cylinder(cylinder_diameter, cylinder_length),
            22 * np.pi,
            "Test related to static methods failed",
        )
        self.assertEqual(
            Fuselage.volume_cylinder(cylinder_diameter, cylinder_length),
            10 * np.pi,
            "Test related to static methods failed",
        )
        self.assertEqual(
            Fuselage.area_sphere(sphere_diameter),
            16 * np.pi,
            "Test related to static methods failed",
        )
        self.assertEqual(
            Fuselage.volume_sphere(sphere_diameter),
            np.pi * 32 / 3,
            "Test related to static methods failed",
        )
